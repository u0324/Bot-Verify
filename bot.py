...
# ==========================================
# 1. AIロジック (的中判定用ロジック含む)
# ==========================================
def analyze_logic(target_df=None):
    # 特定の時点までのデータで予測を行いたい場合に使用
    ...
    train_df = df.dropna()
    if len(train_df) < 2: return "蓄積中", 0, 50, 0.0
...
def get_full_analysis():
    # 通常の/prediction用
    df = load_history()
    status, diff, rsi, score = "方向感の探り合い ➡️", 0, 50, 0.0
...
def handle_show_data_async(token, application_id):
    ...
    if df.empty:
        ...
        for i in range(len(display_df)):
            ...
            lines.append(f"📅 {ts} | 価格: **{int(current_row['price'])}**{hit_mark}")
...
def register_commands():
    ...
    commands = [
        ...
        {"name": "show_data", "description": "保存したデータの履歴(10件)と的中判定を表示します"},
        {"name": "delete_latest", "description": "最新のデータ1件を削除します"},
        {"name": "anime", "description": "人気アニメ情報を表示します", "options": [{"name": "season", "description": "季節", "type": 3, "choices": [{"name":"春","value":"spring"},{"name":"夏","value":"summer"},{"name":"秋","value":"fall"},{"name":"冬","value":"winter"}]}]},

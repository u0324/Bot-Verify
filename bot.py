...
def init_db():
    ...
    with conn.cursor() as cur:
        ...
        # 画像のエラー「column prediction_price does not exist」を解消する自動工事
        cur.execute("""
            DO $$ 
            BEGIN 
...
# ==========================================
# 1. AIロジック (元のアルゴリズムを完全維持 ＋ クラッシュ回避)
# ==========================================
...
def get_full_analysis():
    ...
    train_df = df.dropna()
    if len(train_df) < 2: return "学習準備中", 0, 50, 0.0
    ...
    try:
        ...
        if df.empty:
            ...
            for i, row in enumerate(display_df.itertuples()):
                ...
                idx = row.Index
                hit_mark, pred_info, status_text = "", "", ""

                if i == 0:
                    status_text = " (次回の結果待ち)"
                elif idx > 0:
                    # 前回の入力時にDBへ保存していた「予言」を呼び出す
                    prev_pred = df.iloc[idx-1]['prediction_price']
                    if prev_pred and not np.isnan(prev_pred):
                        pred_info = f" (予:{int(prev_pred)})"
                        # 誤差±1を的中とする
                        hit_mark = " ✅" if abs(row.price - prev_pred) <= 1 else " ❌"

                lines.append(f"📁 {ts} | 価格: **{int(row.price)}**{pred_info}{hit_mark}{status_text}")
            
            data_list = "\n".join(lines)
            embeds = [{"title": "データ履歴 (的中判定)", "description": data_list, "color": 0x2ecc71, "footer": {"text": "✅=的中 / ❌=外れ / (予:)=予測値"}}]
...
def register_commands():
    ...
    commands = [
        ...
        {"name": "show_data", "description": "的中判定を表示します"},
        {"name": "delete_latest", "description": "最新データを削除します"},
        {"name": "anime", "description": "アニメ情報を表示", "options": [{"name": "season", "description": "季節", "type": 3, "choices": [{"name":"春","value":"spring"},{"name":"夏","value":"summer"},{"name":"秋","value":"fall"},{"name":"冬","value":"winter"}]}]},
        {"name": "service", "description": "アニメを検索", "options": [{"name": "work_name", "description": "作品名", "type": 3, "required": True}]}
    ]

...
def handle_yoso_prediction(token, application_id):
    ...
    if price and current_history:
        ...
        json_data = {"content": "☕ 制限中か、データ取得に失敗しました。"}

    url = f"https://discord.com/api/v10/webhooks/{application_id}/{token}/messages/@original"
    requests.patch(url, json=json_data)

# ==========================================
# 3. Flask Endpoint (自分専用制限付き)
# ==========================================
...
def interactions():
    ...
    if data.get('type') == InteractionType.APPLICATION_COMMAND:
        ...
        # 【重要】ここにあなたのDiscordユーザーID(数字)を入れてください
        ALLOWED_USER_ID = '1421704357983813744' 

        if user_id != ALLOWED_USER_ID:
            return jsonify({
                'type': InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
                'data': {'content': "⚠️ このBotのコマンドは管理者専用にロックされています。"}
            })
        # ------------------------------------------

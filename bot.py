...
def init_db():
    ...
    with conn.cursor() as cur:
        ...
        # 画像のエラー(column "prediction_price" does not exist)を自動修正
        cur.execute("""
            DO $$ 
            BEGIN 
...
def save_price(price, pred_price=None):
    ...
    conn = get_db_connection()
    with conn.cursor() as cur:
        # 新しい列 prediction_price にも保存できるように修正
        ...
        if df.empty:
            ...
            for i, row in enumerate(display_df.itertuples()):
                ...
                if i == 0:
                    ...
                    # 前回の入力時に保存された「予測値」を取得
                    prev_prediction = df.iloc[idx-1]['prediction_price']
                    if prev_prediction and not np.isnan(prev_prediction):
            ...
            data_list = "\n".join(lines)
            embeds = [{"title": "データ履歴", "description": data_list, "color": 0x2ecc71, "footer": {"text": "✅=的中 / ❌=外れ / (予:)=予測値"}}]
...
def register_commands():
    ...
    commands = [
        ...
        {"name": "show_data", "description": "最新10件のデータと的中判定を表示します"},
        {"name": "delete_latest", "description": "最新データを削除します"},
        {"name": "anime", "description": "アニメ情報を表示します", "options": [{"name": "season", "description": "季節", "type": 3, "choices": [{"name":"春","value":"spring"},{"name":"夏","value":"summer"},{"name":"秋","value":"fall"},{"name":"冬","value":"winter"}]}]},

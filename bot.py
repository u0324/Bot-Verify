...
def get_full_analysis():
    df = load_history()
    # ログのエラー（0 samples）を回避：データが7件未満なら計算しない
    if len(df) < 7: 
...
def register_commands():
    ...
    commands = [
        {"name": "prediction", "description": "株価を予測します", "options": [{"name": "price", "description": "価格", "type": 4, "required": True}]},
